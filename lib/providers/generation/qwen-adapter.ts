...
export class QwenGenerationAdapter implements GenerationProviderAdapter {
...
function makeResult<T>(model: string, raw: Raw, startedAt: number, data: T): StructuredGenerationResult<T> { return { provider: "QWEN", model, data, usage: { inputTokens: raw.inputTokens, outputTokens: raw.outputTokens }, providerRequestId: raw.requestId, durationMs: Date.now() - startedAt }; }
...
function assertSupported(provider: "QWEN", model: string) { if (!getSupportedModel(provider, model)) throw new ProviderExecutionError("USER_MODEL_UNSUPPORTED", provider, "GENERATION", "RealNeed 尚未验证这个模型的结构化输出能力。", false, true, 422); }
